...
// Campos editables de cada etapa de cierre, con su ícono y unidad.
const FIELDS: { key: keyof ClosingStage; label: string; unit: string; icon: string }[] = [
    { key: 'inicio', label: 'Inicio', unit: 'mm', icon: 'straighten' },
    { key: 'velocidad', label: 'Velocidad', unit: 'mm/s', icon: 'speed' },
    { key: 'torqueMax', label: 'Torque Máx', unit: '%', icon: 'compress' },
];
...
export const ClampParameters: React.FC = () => {
    ...
    const snapshotRef = useRef<ClosingStage[]>([]);

    useEffect(() => {
        repo.getClosingProfile()
            .then(setStages)
            .catch(() => {})
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            const saved = await repo.saveClosingProfile(stages);
            setStages(saved);
            setIsEditing(false);
        } catch (err) {
            console.error('Error al guardar el perfil de cierre:', err);
        } finally {
            setSaving(false);
        }
    };
        ...
        <div className="flex-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-full">
            <div className="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0 flex items-center justify-between">
                <h3 className="font-bold flex items-center gap-2 uppercase text-xs tracking-wider">
                    <span className="material-icons text-primary text-sm">settings</span> Parámetros de Etapa de Cierre
                </h3>
                {!isEditing ? (
                    <button
                        type="button"
            ...
            <div className="flex-1 min-h-0 overflow-auto custom-scrollbar p-6">
                {loading && stages.length === 0 ? (
                    <div className="flex justify-center items-center h-full text-slate-500 text-sm gap-2">
                        <span className="material-icons animate-spin">refresh</span> Cargando perfil de cierre...
                    </div>
                ) : stages.length === 0 ? (
